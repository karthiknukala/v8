...
#if V8_TARGET_CHERI
      // We only push lr rather than lr + padregc on CHERI because lr is going
  ...
  switch (arch_opcode) {
    ...
    case kArchCallWasmFunctionIndirect: {
      if (instr->InputAt(0)->IsImmediate()) {
        ...
        __ Call(i.InputCapabilityRegister(0));
      }
      RecordCallPosition(instr);
      frame_access_state()->ClearSPDelta();
      break;
    ...
    case kArchTailCallWasmIndirect: {
      if (instr->InputAt(0)->IsImmediate()) {
        ...
        if (arch_opcode == kArchTailCallWasmIndirect) {
          __ CallWasmCodePointer(
              c17, i.InputInt64(instr->WasmSignatureHashInputIndex()),
...
#else
      // XXX(ds815): A compiler pass to remove this all together?
      // Always true.
    ...
    case kArchStackSlot: {
      ...
      DCHECK_IMPLIES(V8_TARGET_CHERI_BOOL,
                     instr->OutputAt(0)->IsCapabilityRegister());
    ...
    case kArm64LdrDecompressTaggedSigned:
      RecordTrapInfoIfNeeded(zone(), this, opcode, instr, __ pc_offset());
      __ DecompressTaggedSigned(i.OutputRegisterCapability(), i.MemoryOperand());
      break;
    ...
    case kArm64LdrDecodeSandboxedPointer:
      __ LoadSandboxedPointerField(i.OutputRegisterCapability(),
                                   i.MemoryOperand());
      break;
    case kArm64Str:
      RecordTrapInfoIfNeeded(zone(), this, opcode, instr, __ pc_offset());
...
#define SIMD_DESTRUCTIVE_BINOP_LANE_SIZE_CASE(Op, Instr)               \
  case Op: {                                                           \
    ...
    case kArm64Cpy: {
      ...
      Register dst = i.InputRegisterCapability(0);
      Register src = i.InputRegisterCapability(1);
  ...
  if (frame_access_state()->has_frame()) {
...
#if V8_TARGET_CHERI
      static_assert(StandardFrameConstants::kFixedFrameSize % 16 == 0);
#else
    ...
    if (info()->IsWasm() && required_slots * kSystemPointerSize > 4 * KB) {
      ...
      if (v8_flags.experimental_wasm_growable_stacks) {
        CPURegList regs_to_save(kCRegSizeInBits, RegList{});
...
#if V8_TARGET_CHERI
    static_assert((kDRegSizeInBits * 2) == kCRegSizeInBits);
...
void CodeGenerator::Pop(InstructionOperand* dest, MachineRepresentation rep) {
  ...
  if (dest->IsRegister()) {
    ...
    if (dest->IsCapabilityRegister()) {
      __ Pop(g.ToCapabilityRegister(dest), padregc);
    } else {
    ...
    frame_access_state()->IncreaseSPDelta(-dropped_slots);
    __ Add(csp, csp, Operand(dropped_slots * kSystemPointerSize));
  }
...
#if V8_TARGET_CHERI
    unsigned src_size_log2 = CalcLSDataSizeLog2(LDR_c, true);
#else
    unsigned src_size_log2 = CalcLSDataSizeLog2(LDR_x);
#endif
    int64_t dst_offset = dst.offset();
#if V8_TARGET_CHERI
    unsigned dst_size_log2 = CalcLSDataSizeLog2(STR_c, true);
#else
