...
DirectHandle<HeapObject> RegExpMacroAssemblerARM64::GetCode(
...

  {
    ...
    CallCheckStackGuardState(c10, extra_space_for_variables);
  ...
  __ Sub(x10, input_start().X(), input_end().X());
  if (v8_flags.debug_code) {
  ...
  if (success_label_.is_linked()) {
    ...
    if (num_saved_registers_ > 0) {
      ...
      // Get string length.
      __ Sub(x10, input_end().X(), input_start().X());
      if (v8_flags.debug_code) {
        // Check that the size of the input string chars is in range.
...
void RegExpMacroAssemblerARM64::Break() { __ DebugBreak(); }
...
void RegExpMacroAssemblerARM64::CallCheckStackGuardState(Register scratch,
  ...
  __ Mov(c1, Operand(masm_->CodeObject()));
