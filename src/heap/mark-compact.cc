...
class RecordMigratedSlotVisitor
 ...
 public:
  ...
  inline void VisitPointer(Tagged<HeapObject> host, ObjectSlot p) final {
    Tagged<MaybeObject> maybe_object = p.load(cage_base());
#ifdef __CHERI_PURE_CAPABILITY__
    if (!V8_CHERI_TAG_GET(maybe_object.ptr()) &&
        (maybe_object.ptr() & kZapValue) == maybe_object.ptr()) {
      return;
    }
    DCHECK_IMPLIES(!maybe_object.IsSmi(), V8_CHERI_TAG_GET(maybe_object.ptr()));
#endif  // __CHERI_PURE_CAPABILITY__
    DCHECK(!HasWeakHeapObjectTag(p.load(cage_base())));
    RecordMigratedSlot(host, maybe_object, p.address());
  }
