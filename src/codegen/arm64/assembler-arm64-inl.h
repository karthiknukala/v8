...
void WritableRelocInfo::apply(ScaledInt delta) {
  ...
  if (RelocInfo::IsInternalReference(rmode_)) {
    ...
    jit_allocation_.WriteValue<intptr_t>(pc_, internal_ref);
...
inline void CPURegList::Combine(const CPURegList& other) {
  ...
  DCHECK_IMPLIES(!V8_TARGET_CHERI_BOOL, other.RegisterSizeInBits() == size_);
  list_ |= other.list_;
}
...
Operand Operand::ToX() const {
  if (IsShiftedRegister()) {
    DCHECK_IMPLIES(V8_TARGET_CHERI_BOOL, reg_.Is128Bits() || reg_.Is64Bits());
...
MemOperand::MemOperand(Register base, int64_t offset, AddrMode addrmode)
    : base_(base),
      ...
      extend_(NO_EXTEND),
      shift_amount_(0) {
  DCHECK((base.Is64Bits() || base.IsC()) && !base.IsZero());
}
...
MemOperand::MemOperand(Register base, Register regoffset, Shift shift,
    ...
    : base_(base),
      ...
      shift_amount_(shift_amount) {
  DCHECK((base.Is64Bits() || base.IsC()) && !base.IsZero());
  DCHECK((regoffset.Is64Bits() || regoffset.IsC()) && !regoffset.IsSP());
  DCHECK(shift == LSL);
}
...
MemOperand::MemOperand(Register base, const Operand& offset, AddrMode addrmode)
  ...
  if (offset.IsImmediate()) {
    ...
    DCHECK((regoffset_.Is64Bits() || regoffset_.IsC()) &&
           !regoffset_.IsSP());
    DCHECK(shift_ == LSL);
  } else {
...
#if V8_TARGET_CHERI
inline void Assembler::LoadStoreCapScaledImmOffset(Instr memop, int offset,
                                                   unsigned size) {
...
Instr Assembler::ScbndsImmLiteral(int imm6) {
  CHECK(is_uint6(imm6));
...
unsigned Assembler::CalcLSDataSizeLog2(LoadStoreOp op, bool is_cap) {
