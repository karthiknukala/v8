...
bool ConstantPool::ShouldEmitNow(Jump require_jump, size_t margin) const {
  ...
  if (EntryPtrCount() != 0) {
    ...
    bool opportune_emission_without_jump =
        ...
        (dist_ptr >= kOpportunityDistToPool64);
    bool approximate_distance_exceeded = dist_ptr >= kApproxDistToPool64;
...
int ConstantPool::ComputeSize(Jump require_jump,
  ...
  const size_t size = size_up_to_marker + assm_->pc_offset();
  const size_t alignment =
      require_alignment == Alignment::kRequired
          ? RoundUp(size, kSystemPointerSize) - size_up_to_marker
          : 0;
