 ...
 public:
  ...
  void StartFunctionBody(FullDecoder* decoder, Control* block) {
    ...
    if (v8_flags.debug_code) {
      ...
      Register instance =
          pinned.set(LoadInstanceIntoRegister(pinned, scratch)).C();
      // Load the map.
...
#define FUZZER_HEAVY_INSTRUCTION                      \
  ...
  void GenerateCCallWithStackBuffer(const LiftoffRegister* result_regs,
    ...
    for (const VarState& arg : args) {
      int current_size = value_kind_size(arg.kind());
#if V8_TARGET_CHERI
...
#undef RUNTIME_STUB_FOR_TRAP
  ...
  Register BoundsCheckMem(FullDecoder* decoder, const WasmMemory* memory,
    ...
    if (memory->index == 0) {
      LOAD_INSTANCE_FIELD(mem_size.gp(), Memory0Size, kSystemPointerAddrSize,
                          pinned);
    } else {
      LOAD_PROTECTED_PTR_INSTANCE_FIELD(mem_size.gp(), MemoryBasesAndSizes,
                                        pinned);
      // FIXME(ds815): Check if this is sensible.
      int buffer_offset =
  ...
  void TraceMemoryOperation(bool is_store, uint32_t mem_index,
    ...
    __ Store(info.gp().C(), no_reg, offsetof(MemoryTracingInfo, mem_index),
             data, StoreType::kI32Store, pinned);
    __ LoadConstant(data, WasmValue(is_store ? 1 : 0));
  ...
  void CurrentMemoryPages(FullDecoder* /* decoder */,
    ...
    if (imm.index == 0) {
      ...
      LOAD_PROTECTED_PTR_INSTANCE_FIELD(mem_size.gp().C(), MemoryBasesAndSizes,
                                        pinned);
      int buffer_offset =
          wasm::ObjectAccess::ToTagged(OFFSET_OF_DATA_START(ByteArray)) +
          kSystemPointerSize * (imm.index * 2 + 1);
      __ LoadFullPointer(mem_size.gp(), mem_size.gp().C(), buffer_offset);
    }
    // Convert bytes to pages.
    __ emit_ptrsize_shri(mem_size.gp().C(), mem_size.gp().C(),
                         kWasmPageSizeLog2);
...
#undef CASE_SIMD_REPLACE_LANE_OP
  ...
  void Throw(FullDecoder* decoder, const TagIndexImmediate& imm,
    ...
    CallBuiltin(
        ...
        {VarState{kCapability64Kind, LiftoffRegister{encoded_size_reg}, 0}},
        decoder->position());
#else
    ...
    Register instance_data = __ cache_state() -> cached_instance_data.C();
    if (instance_data == no_reg) {
      instance_data = __ GetUnusedRegister(kGpReg, pinned).gp().C();
      __ LoadInstanceDataFromFrame(instance_data);
    }
#if V8_TARGET_CHERI
    CallBuiltin(Builtin::kWasmThrow,
                MakeSig::Params(kCapability64Kind, kCapability64Kind, kRef),
                {VarState{kCapability64Kind, exception_tag, 0},
                 VarState{kCapability64Kind, values_array, 0},
...
// painfully few registers available on ia32.
  ...
  VarState IndexToVarStateSaturating(int stack_index, LiftoffRegList* pinned) {
    ...
    // For memory32 on a 32-bit system or memory64 on a 64-bit system, there is
    // nothing to do.
    if ((kSystemPointerAddrSize == kInt64Size) == is_mem64) {
      if (slot.is_reg()) pinned->set(slot.reg());
      return slot;
    }
    ...
    DCHECK_EQ(kSystemPointerAddrSize, kInt32Size);
    pinned->set(reg.low());
    Label ok;
    FREEZE_STATE(frozen);
  ...
  void StructNew(FullDecoder* decoder, const StructIndexImmediate& imm,
    ...
    for (uint32_t i = imm.struct_type->field_count(); i > 0;) {
      ...
      StoreObjectField(decoder, obj.gp().C(), no_reg, offset, value, false,
                       pinned, field_type.kind(), write_barrier);
      pinned.clear(value);
    }
  ...
  void StructGet(FullDecoder* decoder, const Value& struct_obj,
    ...
    LoadObjectField(decoder, value, obj.gp().C(), no_reg, offset, field_kind,
  ...
  void StructAtomicGet(FullDecoder* decoder, const Value& struct_obj,
    ...
    if (explicit_check) {
      MaybeEmitNullCheck(decoder, obj.gp().C(), pinned, struct_obj.type);
    }
    LiftoffRegister value =
        __ GetUnusedRegister(reg_class_for(field_kind), pinned);
    LoadAtomicObjectField(decoder, value, obj.gp().C(), no_reg, offset,
                          field_kind, is_signed, implicit_check, memory_order,
                          pinned);
    __ PushRegister(unpacked(field_kind), value);
  }
  ...
  void StructSet(FullDecoder* decoder, const Value& struct_obj,
    ...
    StoreObjectField(decoder, obj.gp().C(), no_reg, offset, value,
                     implicit_check, pinned, field_kind);
  }
  ...
  void StructAtomicSet(FullDecoder* decoder, const Value& struct_obj,
    ...
    StoreAtomicObjectField(decoder, obj.gp().C(), no_reg, offset, value,
  ...
  void ArrayFill(FullDecoder* decoder, ArrayIndexImmediate& imm,
    ...
    FUZZER_HEAVY_INSTRUCTION;
    {
      ...
      bool implicit_null_check =
          ...
          null_check_strategy_ == compiler::NullCheckStrategy::kTrapHandler;
      LoadObjectField(decoder, array_length, array_reg.gp().C(), no_reg,
  ...
  void ArrayGet(FullDecoder* decoder, const Value& array_obj,
    ...
    LoadObjectField(decoder, value, array.gp().C(), index.gp(),
                    wasm::ObjectAccess::ToTagged(WasmArray::kHeaderSize),
  ...
  void ArraySet(FullDecoder* decoder, const Value& array_obj,
    ...
    StoreObjectField(decoder, array.gp().C(), index.gp(),
                     wasm::ObjectAccess::ToTagged(WasmArray::kHeaderSize),
  ...
  void ArrayLen(FullDecoder* decoder, const Value& array_obj, Value* result) {
    ...
    bool implicit_null_check =
        array_obj.type.is_nullable() &&
        null_check_strategy_ == compiler::NullCheckStrategy::kTrapHandler;
    LoadObjectField(decoder, len, obj.gp().C(), no_reg, kLengthOffset, kI32,
  ...
  void ArrayNewFixed(FullDecoder* decoder, const ArrayIndexImmediate& array_imm,
    ...
    for (int i = elem_count - 1; i >= 0; i--) {
      ...
      StoreObjectField(decoder, array.gp().C(), no_reg,
  ...
  LiftoffRegister RttCanon(FullDecoder* decoder, ModuleTypeIndex type_index,
    ...
    if (is_shared) {
      LOAD_PROTECTED_PTR_INSTANCE_FIELD(rtt.gp().C(), SharedPart, pinned);
      LOAD_TAGGED_PTR_FROM_INSTANCE(rtt.gp().C(), ManagedObjectMaps,
                                    rtt.gp().C())
    } else {
  ...
  void RefTest(FullDecoder* decoder, HeapType target_type, const Value& obj,
...

    {
      ...
      SubtypeCheck(decoder, obj_reg.gp().C(), obj.type, rtt_reg.gp().C(),
  ...
  void RefCastImpl(FullDecoder* decoder, ValueType target_type,
...

    {
      ...
      OolTrapLabel trap =
          AddOutOfLineTrap(decoder, Builtin::kThrowWasmTrapIllegalCast);
      SubtypeCheck(decoder, obj_reg.gp().C(), obj.type, rtt.gp().C(),
  ...
  void BrOnCastImpl(FullDecoder* decoder, HeapType target_type,
    ...
    SubtypeCheck(decoder, obj_reg.gp().C(), obj.type, rtt.gp().C(), target_type,
  ...
  void Initialize(TypeCheck& check, FullDecoder* decoder, PopOrPeek pop_or_peek,
    ...
    if (pop_or_peek == kPop) {
      ...
      check.obj_reg = pinned.set(__ PeekToRegister(0, pinned)).gp().C();
    }
    check.tmp = pinned.set(__ GetUnusedRegister(kGpReg, pinned)).gp().C();
  ...
  void StringMeasureWtf16(FullDecoder* decoder, const Value& str,
    ...
    MaybeEmitNullCheck(decoder, string_reg.gp(), pinned, str.type);
    LiftoffRegister value = __ GetUnusedRegister(kGpReg, pinned);
    LoadObjectField(decoder, value, string_reg.gp().C(), no_reg,
 ...
 private:
  void CallDirect(FullDecoder* decoder, const CallFunctionImmediate& imm,
    ...
    if (needs_indirect_call) {
      ...
      Register implicit_arg =
          pinned.set(__ GetUnusedRegister(kGpReg, pinned)).gp().C();
      Register target = pinned.set(__ GetUnusedRegister(kGpReg, pinned)).gp().C();
  ...
  void CallIndirectImpl(FullDecoder* decoder, const CallIndirectImmediate& imm,
    ...
    if (imm.table_imm.index == 0) {
      ...
      LOAD_PROTECTED_PTR_INSTANCE_FIELD(dispatch_table.gp_reg().C(),
                                        DispatchTable0, pinned);
    } else {
      // Load the dispatch table from the ProtectedFixedArray of all dispatch
      // tables.
      Register dispatch_tables = dispatch_table.gp_reg().C();
      LOAD_PROTECTED_PTR_INSTANCE_FIELD(dispatch_tables, DispatchTables,
                                        pinned);
      __ LoadProtectedPointer(dispatch_table.gp_reg().C(), dispatch_tables,
...

    {
      ...
      if (statically_oob) {
        ...
        __ Load(table_size.reg(), dispatch_table.gp_reg().C(), no_reg,
    ...
    if (needs_type_check || needs_null_check) {
      ...
      if (!needs_type_check) {
        ...
        __ LoadFullPointer(
            real_rtt.gp_reg().C(), kRootRegister,
            IsolateData::root_slot_offset(RootIndex::kWasmCanonicalRtts));
        __ LoadTaggedPointer(
            real_rtt.gp_reg().C(), real_rtt.gp_reg().C(), real_sig_id.gp_reg(),
        ...
        __ LoadTaggedPointer(type_info.gp_reg().C(), type_info.gp_reg().C(),
        ...
        __ LoadTaggedPointer(
            maybe_match.gp_reg().C(), maybe_match.gp_reg().C(), no_reg,
            ObjectAccess::ToTagged(WasmTypeInfo::kSupertypesOffset +
                                   rtt_depth * kTaggedSize));
        ...
        LOAD_TAGGED_PTR_INSTANCE_FIELD(formal_rtt.gp_reg(), ManagedObjectMaps,
                                       kGpCacheRegList);
        __ LoadTaggedPointer(
            formal_rtt.gp_reg().C(), formal_rtt.gp_reg().C(), no_reg,
            wasm::ObjectAccess::ElementOffsetInTaggedFixedArray(
  ...
  void CallRefImpl(FullDecoder* decoder, ValueType func_ref_type,
    ...
    if (v8_flags.wasm_inlining) {
...
#if V8_TARGET_CHERI
                  MakeSig::Returns(kCapability64Kind, kCapability64Kind)
                      .Params(kRef, kI32, kRef),
#else
                  MakeSig::Returns(kIntPtrKind, kIntPtrKind)
                      .Params(kRef, kI32, kRef),
#endif
      ...
      LiftoffRegList pinned;
      Register func_ref = pinned.set(__ PopToModifiableRegister(pinned)).gp().C();
      MaybeEmitNullCheck(decoder, func_ref, pinned, func_ref_type);
      implicit_arg_reg =
          pinned.set(__ GetUnusedRegister(kGpReg, pinned)).gp().C();
      target_reg = pinned.set(__ GetUnusedRegister(kGpReg, pinned)).gp().C();
  ...
  void LoadObjectField(FullDecoder* decoder, LiftoffRegister dst, Register src,
    ...
    uint32_t protected_load_pc = 0;
    if (is_reference(kind)) {
      __ LoadTaggedPointer(dst.gp().C(), src.C(), offset_reg, offset,
  ...
  void ArrayFillImpl(FullDecoder* decoder, LiftoffRegList pinned,
    ...
    StoreObjectField(decoder, obj.gp().C(), offset.gp(), 0, value, false,
                     pinned, elem_kind, skip_write_barrier);
  ...
  V8_INLINE Register LoadInstanceIntoRegister(LiftoffRegList pinned,
    ...
    if (V8_TARGET_CHERI_BOOL && instance.IsRegister()) {
      instance = instance.C();
    }
    return instance;
  }
  ...
  LoadInstanceIntoRegister_Slow(LiftoffRegList pinned, Register fallback) {
    ...
    Register instance = __ cache_state()->TrySetCachedInstanceRegister(
        pinned | LiftoffRegList{fallback}).C();
