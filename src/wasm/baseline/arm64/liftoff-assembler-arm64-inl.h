...
void LiftoffAssembler::PrepareTailCall(int num_callee_stack_params,
  ...
  Sub(c16, c16, stack_param_delta * kSystemPointerSize);
  int slot_count = num_callee_stack_params;
  for (int i = slot_count - 1; i >= 0; --i) {
    ldr(scratch, MemOperand(csp, i * kSystemPointerSize));
    str(scratch, MemOperand(c16, i * kSystemPointerSize));
  }
...
Register LiftoffAssembler::LoadOldFramePointer() {
  ...
  Ldr(old_fp.gp().C(), MemOperand(fp, TypedFrameConstants::kFrameTypeOffset));
  Cmp(old_fp.gp(),
  ...
  if (old_fp.gp().C() != kReturnRegister0) {
    Mov(old_fp.gp().C(), kReturnRegister0);
  }
  PopRegisters(regs_to_save);
...
void LiftoffAssembler::LoadTaggedPointerFromInstance(Register dst,
                                                     ...
                                                     int offset) {
  DCHECK_LE(0, offset);
  DCHECK_IMPLIES(V8_TARGET_CHERI_BOOL && !COMPRESS_POINTERS_BOOL, dst.IsC());
  DCHECK_IMPLIES(V8_TARGET_CHERI_BOOL, instance.IsC());
...
void LiftoffAssembler::StoreTaggedPointer(Register dst_addr,
  ...
  UseScratchRegisterScope temps(this);
  DCHECK_IMPLIES(V8_TARGET_CHERI_BOOL, dst_addr.IsC());
  DCHECK_IMPLIES(V8_TARGET_CHERI_BOOL, src.IsC());
  ...
  }
  {
    ...
    StoreTaggedField(src, MemOperand(dst_addr.C(), offset_op));
  }
...
inline Register CalculateActualAddress(LiftoffAssembler* lasm,
  ...
  DCHECK_IMPLIES(V8_TARGET_CHERI_BOOL, addr_reg.IsC());
  if (offset_reg == no_reg && offset_imm == 0) return addr_reg;
  Register result = temps.AcquireC();
...
void LiftoffAssembler::StoreCallerFrameSlot(LiftoffRegister src,
  ...
  if (cpu_reg.IsRegister()) {
    Str(cpu_reg.C(), MemOperand(frame_pointer, offset));
  } else {
    Str(cpu_reg, MemOperand(frame_pointer, offset));
  }
}
...
void LiftoffAssembler::StackCheck(Label* ool_code) {
  UseScratchRegisterScope temps(this);
  Register limit_address = temps.AcquireC();
...
void LiftoffAssembler::TailCallIndirect(
  ...
  temps.Exclude(c17);
  Mov(c17, target);
  CallWasmCodePointer(c17, call_descriptor->signature_hash(),
