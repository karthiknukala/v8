...
#define V8_CHERI_SET_BOUNDS_EXACT(cap, len) \
  __builtin_cheri_bounds_set_exact(cap, len)
...
template <typename T>
constexpr T RoundDown(T x, ssize_t m) {
  static_assert(std::is_integral_v<T>);
  // m must be a power of two.
  DCHECK(m != 0 && ((m & (m - 1)) == 0));
  return x & static_cast<T>(-m);
}
template <ssize_t m, typename T>
...
template <typename T>
constexpr T RoundUp(T x, ssize_t m) {
  static_assert(std::is_integral_v<T>);
  DCHECK_GE(x, 0);
...
#if V8_TARGET_CHERI
#define IF_TARGET_CHERI(V, ...) EXPAND(V(__VA_ARGS__))
#else
#define IF_TARGET_CHERI(V, ...)
#endif
