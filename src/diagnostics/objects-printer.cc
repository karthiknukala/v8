...
void PrintFixedArrayElements(std::ostream& os, Tagged<T> array, int capacity,
  ...
  for (i = 1; i <= capacity; i++) {
    ...
    if (V8_CHERI_PURECAP_BOOL && !V8_CHERI_TAG_GET(previous_value.ptr())) {
      os << " [INVALID]";
    }
    previous_index = i;
    previous_value = value;
...
void JSGeneratorObject::JSGeneratorObjectPrint(std::ostream& os) {
  ...
  if (V8_CHERI_PURECAP_BOOL && !V8_CHERI_TAG_GET(function().ptr())) {
    os << " [INVALID]";
  }
  os << "\n - context: " << Brief(context());
  if (V8_CHERI_PURECAP_BOOL && !V8_CHERI_TAG_GET(context().ptr())) {
    os << " [INVALID]";
  }
  os << "\n - receiver: " << Brief(receiver());
  if (V8_CHERI_PURECAP_BOOL && !V8_CHERI_TAG_GET(receiver().ptr())) {
    os << " [INVALID]";
  }
...
void NativeContext::NativeContextPrint(std::ostream& os) {
  ...
  if (V8_CHERI_PURECAP_BOOL && !V8_CHERI_TAG_GET(microtask_queue())) {
    os << " [INVALID]";
  }
  os << "\n";
}
